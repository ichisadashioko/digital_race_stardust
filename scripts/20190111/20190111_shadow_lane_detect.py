...
class Lane_Detect:
    ...
    def sliding_window(self, img, nwindows=16, margin=20, minpix=1, draw_windows=True, left_color=(0, 0, 255), right_color=(0, 255, 0), thickness=1):
        ...
        if len(leftx) == 0 or len(lefty) == 0:
            # if there are zero pixels in the lane then x = 0y + 0
    ...
    def draw_lanes(self, src_img, plot_dsize, leftx_pts, rightx_pts, ploty, color=(255, 0, 0)):
        stretch_ratio = 0.3
    ...
    def pipeline(self, img):
        ...
        mask_01 = self.binary_HSV(bird_view)
        mask_02 = self.shadow_HSV(bird_view)
        bird_view_mask = cv2.bitwise_or(mask_01, mask_02)

        # bird_view_mask = cv2.rectangle(bird_view_mask, (220, 0), (260, 320), 0) # "remove" the center lane
        s_window, (left_fitx, right_fitx), (left_fit_,right_fit_), ploty = self.sliding_window(bird_view_mask)

        d_line, d_xs, d_ys = self.get_desired_line(
            left_fitx, right_fitx, ploty)

        lines_plot_img = np.zeros_like(s_window, dtype=np.uint8)
        cv2.line(lines_plot_img, (240, 0), (240, 320),
                 color=(0, 0, 255), thickness=1)
        cv2.line(lines_plot_img, (int(d_xs[0]), 0), (int(
            d_xs[len(d_xs)-1]), 320), color=(0, 255, 0), thickness=1)

        draw_lane = self.draw_lanes(
