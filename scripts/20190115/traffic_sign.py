...
class Traffic_Sign:
    ...
    # define region contains traffic sign

    def region_of_interest(self, image):
        height, width = image.shape[:2]
        SKYLINE = int(height * 0.45)
        vertices = np.array([[(width/2, SKYLINE), (width/2, 0), (width, 0), (width, SKYLINE)]], np.int32)
                ...
                if len(conts1) != 0 and len(conts2) != 0:
                    ...
                    if self.testing:
                        ...
                        if self.sign == 1:
                            ...
                            text = 'left'
                        cv2.putText(image, text, (x, y-3), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 255))
                        print(self.sign)
